<template>
  <el-container>
    <el-header>
      <el-button type="primary" round @click="dialogFormAddRouteVisible = true, addRouteBefore()">添加新路由</el-button>
    </el-header>
    <el-main>
      <el-table :data="tableData" stripe border style="width: 100%">
        <el-table-column fixed prop="id" label="#" width="50px"/>
        ...
        <el-table-column fixed="right" label="操作" width="120">
          <template #default="scope">
            <el-button link type="primary" size="small" @click="dialogFormModifyRoute = true, modifyRouteBefore(scope.row)">修改</el-button>
            <el-button link type="primary" size="small" @click="dialogDeleteRoute = true, deleteRouteBefore(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination background layout="prev, pager, next" :total="total" @current-change="change" />
    </el-main>
    <el-dialog v-model="dialogDeleteRoute" title="删除路由">
      ...
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogDeleteRoute = false,deleteRouteMethod()">确认删除路由</el-button>
      </span>
      </template>
    </el-dialog>
    <el-dialog v-model="dialogFormModifyRoute" title="修改新路由">
      <el-form :model="modifyRouteForm">
        ...
        <el-form-item label="父组件名" :label-width="formLabelWidth">
          <el-select v-model="modifyRouteForm.parentId" placeholder="请选择父组件名" size="large">
            <el-option
        ...
        <el-form-item label="权限组" :label-width="formLabelWidth">
          <el-select v-model="modifyRouteForm.auth" placeholder="请选择权限" size="large">
      ...
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogFormModifyRoute = false,modifyRouteMethod()">确认修改路由</el-button>
      </span>
      </template>
    </el-dialog>
    <el-dialog v-model="dialogFormAddRouteVisible" title="添加新路由">
      <el-form :model="addRouteForm">
        ...
        <el-form-item label="父组件名(没有不选)" :label-width="formLabelWidth">
          <el-select v-model="addRouteForm.parentId" placeholder="请选择父组件名" size="large">
            <el-option
                v-for="item in routeName"
                :key="item.id"
                :label="item.name"
        ...
        <el-form-item label="权限组" :label-width="formLabelWidth">
          <el-select v-model="addRouteForm.auth" placeholder="请选择权限" size="large">
            <el-option value="user" label="user"/>
            <el-option value="admin" label="admin"/>
            <el-option value="guest" label="guest"/>
          </el-select>
      ...
      <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogFormAddRouteVisible = false">取消添加新路由</el-button>
        <el-button type="primary" @click="dialogFormAddRouteVisible = false,addRouteMethod()">确认添加新路由</el-button>
      </span>
      </template>
    </el-dialog>
  </el-container>
...
import {getRouteName, getRoutes, addRoute, updateStatus, updateRoute} from "../../../api/Manager";
...
const addRouteForm = reactive<Route>({path: '', name: '', parentId: '', component: '', title: '', auth: '', status: 0, remark: '', updateId: -1, createId: store.userinfo!.id, createTime: '', updateTime: '', id: -1});
const formLabelWidth = '140px';
const routeName = ref<Array<{id: number, name: string}>>([])
const modifyRouteForm = reactive<Route>({path: '', name: '', parentId: '', component: '', title: '', auth: '', status: 0, remark: '', updateId: -1, createId: store.userinfo!.id, createTime: '', updateTime: '', id: -1})
...
const modifyRouteMethod = async () => {
  ...
  data.updateId = store.userinfo!.id;
