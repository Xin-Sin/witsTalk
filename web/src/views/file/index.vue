<template>
        ...
        <el-main>
          ...
          <div v-else>
            ...
            <el-button
                ...
                round
                size="small"
                @click="downloadFile(selectFile.md5,selectFile.name)"
            >文件下载</el-button>
            <el-divider/>
            <el-image
                v-if="showImg !== '' && !showAudio"
                style="width: 750px; height: 670px"
                :src="showImg"
                fit="fill"
            />
            <el-input
  ...
  const headers = reactive <{"Access-Token": string}>
      ({"Access-Token": window.localStorage.getItem("token") as string});
  ...
  const fileUploadProgress = (evt: UploadProgressEvent) => {
    if (evt.percent === 100){
      if (messageHandler1.value){
        messageHandler1.value =
            ElMessage.warning({"message":"文件已交由后端进行处理,请耐心等待,", "duration":0});
      }
    }
  ...
  // 文件上传前回调方法
  // 不做文件相关处理，对用户进行响应提示
  const fileUploadBefore = (): Awaitable<boolean> => {
    if (messageHandler1.value){
      messageHandler.value =
          ElMessage.warning({"message":"文件上传中,请不要关闭该窗口", "duration":0});
    }
  ...
  const fileUploadSuccess = (response: any) => {
    ...
    if (messageHandler.value && messageHandler1.value){
  ...
  const connectedFileUrl = (md5:string, fileName:string) => {
    return "http://localhost:8080/file/api/downloadFile?md5=" +
        md5 +
        "&filename=" +
        fileName +
        "&token=" +
  ...
  const handleCurrentChange = (val: object | undefined) => {
    ...
    if(a[a.length-1] === 'jpg' || a[a.length-1] === 'png'){
      showImg.value =
          connectedFileUrl(selectFile.value?.md5, selectFile.value?.name);
    }
      ...
      // 获取文件内容
      getShowFile(selectFile.value?.md5,
          selectFile.value?.name,
          localStorage.getItem("token") as string).then((res) => {
            showImg.value = '';
