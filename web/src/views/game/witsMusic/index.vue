<template>
  <div ref="div" class="getter">
    <canvas :width="total_width" :height="total_height" class="main-content" ref="canvas" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const mounted = () => {
    ...
    const context: CanvasRenderingContext2D = canvas.value?.getContext("2d") as CanvasRenderingContext2D;
    context.fillStyle = "#FEFFA9";
    context.fillRect(0,0,width,height);
    context.globalCompositeOperation = "destination-in";
    new Line(100,0).draw(context);
    //new Line(200,90).draw(context);
  }
  onMounted(mounted)
</script>
