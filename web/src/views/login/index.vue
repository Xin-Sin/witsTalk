  ...
  <div class="background">
    <div id="window" class="login-window">
      ...
      <div class="inline-div right">
        ...
        <div
            ref="title"
            class="login-window-child title"
            style="margin-top: 5vh;font-size: 20px;overflow: hidden"
        />
        <div
            class="login-window-child input"
...
const Login = () => {
  if (username.value !== '' && password.value !== '') {
    if (verify(username.value as string, 'u') &&
        verify(password.value as string, 'p')
    ){
      // 打开模态框进行图形验证码验证
      ...
      await getRouter("admin").then((res) => {
        for (const datum of res.data.data) {
          ...
          if (datum.parentId === null){
            ...
            router.addRoute(datum.parentId,
                {path: datum.path,
                  name: datum.name,
                  component: () => import("../" + datum.component),
                  meta: {
...
div.background {
  ...
  background-repeat: no-repeat;
  background-size: 100% 100%;
  background-image: linear-gradient(177deg, rgb(59, 53, 89) 15%, rgb(134, 77, 72), rgb(196, 93, 59), rgb(119, 79, 105), rgba(118, 133, 137, 100%), rgba(136, 134, 120, 100%));
}
</style>
